 ...
 */
 export class DependencyGraph<T = any, D = any> {

	protected _nodes = new Set<T>();
	protected _nodeData = new Map<T, D>();
	 ...
	 * @param data
	 */
	public setNodeData(node: T, data?: D) {
		if (data !== undefined) {
			this._nodeData.set(node, data);
		}
		else {
	 ...
	 * @returns
	 */
	public hasNodeData(node: T) {
		return this._nodeData.has(node);
	}
	 ...
	 *
	 * @param node
	 * @param leaves When true, only nodes that do not depend on any other nodes will be returned.
	 * @returns
	 */
	public getDependenciesOf(node: T, leaves = false) {
		if (this.hasNode(node)) {
			const result = new Set<T>();
			const dfs = this.createDFS(this._outgoingEdges, leaves, this.allowCircularDependencies, result);
	 ...
	 *
	 * @param node
	 * @param leaves When true, only nodes that do not have any dependents will be returned.
	 * @returns
	 */
	public getDependentsOf(node: T, leaves = false) {
		if (this.hasNode(node)) {
			const result = new Set<T>();
			const dfs = this.createDFS(this._incomingEdges, leaves, this.allowCircularDependencies, result);
	 ...
	 * @param leaves When true, only nodes that do not depend on any other nodes will be returned.
	 */
	public getOverallOrder(leaves = false) {
		const result = new Set<T>();
		...
		if (!this.allowCircularDependencies) {
			const dfs = this.createDFS(this._outgoingEdges, false, this.allowCircularDependencies, new Set());
	 ...
	 */
	public clone() {
		const graph = new DependencyGraph<T, D>();

		graph.allowCircularDependencies = this.allowCircularDependencies;
		graph._nodes = new Set(this._nodes);
		graph._nodeData = new Map(this._nodeData);
		graph._incomingEdges = new Map(this._incomingEdges);
		graph._outgoingEdges = new Map(this._outgoingEdges);
	...
	*/
	protected createDFS<T>(edges: Map<T, Set<T>>, filterLeaves: boolean, allowCircularDeps: boolean, result: Set<T>) {
	   ...
	   return function (start: T) {
		   ...
		   const todo = new Array<{ node: T, processed: boolean }>();
